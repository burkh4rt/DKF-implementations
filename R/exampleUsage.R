...
fx <- function(x) {
  t(as.matrix(cbind(predict(rf_mdl1, x)$predictions, predict(
    rf_mdl2,
    x
  )$predictions)))
}
...
z_train_preds <- as.matrix(cbind(
  predict(rf_mdl1, x_train_covariance)$predictions,
...
f0 <- fx(x_test[1, ])
Q0 <- Qx(x_test[1, ])
DKF <- DiscriminativeKalmanFilter$new(
  stateModelA = A0, stateModelGamma = Gamma0,
